...
export class PdfService {
  ...
  return new Promise<Blob>((resolve, reject) => {
    ...
    if (inventoryData.length > 0) {
      ...
      documentDefinition = {
        header: { image: this.headerString, fit: [40, 40] as [number, number], alignment: 'center' as Alignment},
        content: [
    ...
    pdfMake.createPdf(documentDefinition).download(`inventory_report_${currentDate}.pdf`);
  })
  }
 ...
 async  generateInventoryReport(inventoryData: Inventory[], currentDate: string): Promise<Blob> {
  ...
  async generateEventsPdf(eventData: Event[], beginDate: Date, endDate: Date): Promise<void> {
    type Alignment = 'left' | 'right' | 'center' | 'justify';
    this.headerString = await this.toBase64('assets/download.png');
...
async generateSupplierOrders(supplierOrderData: SuppOrderAndVATViewModel[]): Promise<Blob> {
  ...
  async generateBlacklistPdf(blacklistData: Blacklist[], currentDate: string): Promise<void> {
    type Alignment = 'left' | 'right' | 'center' | 'justify';
    let documentDefinition: any;
    this.headerString = await this.toBase64('assets/download.png');
    this.toastr.success('Generating...','Report');
    ...
    pdfMake.createPdf(documentDefinition).download(`blacklist_report_${currentDate}.pdf`);
  }
