        ...
        content: [
          ...
          { text: `From: ${formattedBeginDate} To: ${formattedEndDate}`, style: 'subheader' },
          {
            table: {
              ...
              widths: ['auto', 'auto', 'auto', 'auto', 'auto', 'auto'],
              body: [
                ['No.', 'Reference Number', 'Requested On', 'Description', 'Email', 'Status'],
                ...refundData.map((item, index) => [index, item.orderRefNum, item.requestedOn, item.description, item.email, item.status])
              ]
            }
          }
        ],
    ...
    if (supplierOrderData.length > 0) {
      const documentDefinition = {
        content: [
          ...
          { text: `Generated On: ${generatedDate}`, style: 'subheader' },
          {
            table: {
              ...
              body: [
                ['No.', 'SupplierID', 'Wine Name', 'Quantity Ordered', 'Order Date', 'Order Total', 'Status'],
                ...supplierOrderData.map((item, index) => {
                  let status = 'Pending'; // Status for each row
                  if (item.received == true) {
                    status = "Received";
                  } else if (item.paid == true) {
                    status = "Paid";
                  } else if (item.ordered == true) {
                    status = "Ordered";
                  }
  
                  return [index, item.supplierID, item.wineName, item.quantity_Ordered, item.dateOrdered, "R " + item.orderTotal, status];
                })
              ]
            }
          }
        ...
        styles: {
          ...
          subheader: {
            fontSize: 12,
            margin: [0, 6] as [number, number]
          }
        },
        pageOrientation: 'landscape' as const
      };
  
      pdfMake.createPdf(documentDefinition).download(`refunds_report_${generatedDate}.pdf`);
